...
statusbar = ttk.Label(root, text="Welcome to CD-MUSIC PLAYER "
...
def about_us():
    tkinter.messagebox.showinfo('About musyc', 'This is a music player build using Python Tkinter by @Chinmay')
...
def videos():
    ...
    try:
        ...
        y = YouTube(link).streams.first().download()
        print(y)
    except:
...
subMenu = Menu(menubar, tearoff=0)
menubar.add_cascade(label="Download", menu=subMenu)
subMenu.add_command(label="videos", command=videos)
...
root.title("CD-MUSIC PLAYER")
root.iconbitmap(r'musicg.ico')
...
CD-MUSIC PLAYERcould not find the file. Please check again.')
